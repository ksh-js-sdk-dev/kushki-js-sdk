...
export class KushkiGateway implements IKushkiGateway {
  ...
  public constructor(kushkiInstance: Kushki) {
    this.kushkiInstance = kushkiInstance;
  }

  public requestBinInfo = async (body: BinBody): Promise<BinInfoResponse> => {
    try {
  ...
  public requestMerchantSettings =
    async (): Promise<MerchantSettingsResponse> => {
      const url: string = `${this.kushkiInstance.getBaseUrl()}${
      ...
      try {
        const { data } = await axios.get<MerchantSettingsResponse>(url, {
          headers: this._buildHeader(
            this.kushkiInstance.getPublicCredentialId()
          )
        });
  ...
  public requestCybersourceJwt = async (): Promise<CybersourceJwtResponse> => {
    const url: string = `${this.kushkiInstance.getBaseUrl()}${
